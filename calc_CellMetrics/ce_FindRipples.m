...
if isstruct(varargin{1})  % if first arg is a session struct
    ...
    disp(session.extracellular.srLfp)
...
keep = logical(keep); 
disp(p.Results.absoluteThresholds)
% only do if absolute threshold box is checked
...
for i = 1:size(secondPass,1)
	...
	if maxValue > highThresholdFactor % ADD BACK
...
ripples(duration>maxRippleDuration/1000,:) = NaN; % ADD BACK
