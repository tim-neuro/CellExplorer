...
for i=1:length(rxml.child)
    ...
    switch lower(rxml.child(i).tag)
        ...
        case 'region'
            % Parse per-channel region array
            % Used by: session.brainRegions.(regionName).channels = find(strcmp(...))
            if ~isempty(rxml.child(i).value)
                regionStr = strtrim(rxml.child(i).value);
                % Split by spaces and convert to cell array
                xml.region = strsplit(regionStr);
            else
                xml.region = {};
            end
            
        case 'brainregions'
            % Parse brain regions structure for NeuroScope2
            % Provides: xml.brainRegions.(regionName).channels and .electrodeGroups
            if ~isempty(rxml.child(i).child)
                regionList = rxml.child(i).child;
                for regionI = 1:length(regionList)
                    regionName = regionList(regionI).tag;
                    
                    % Parse channels and electrodeGroups for this region
                    for fieldI = 1:length(regionList(regionI).child)
                        fieldName = regionList(regionI).child(fieldI).tag;
                        fieldValue = strtrim(regionList(regionI).child(fieldI).value);
                        
                        if strcmp(fieldName, 'channels')
                            xml.brainRegions.(regionName).channels = str2num(fieldValue) + 1; % Convert to 1-indexed
                        elseif strcmp(fieldName, 'electrodeGroups')
                            xml.brainRegions.(regionName).electrodeGroups = str2num(fieldValue) + 1; % Convert to 1-indexed
                        end
                    end
                end
            end
