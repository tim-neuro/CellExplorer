...
if isfield(sessionInfo,'region')
    ...
    for iRegion = 1:length(regionNames)
        % changed by Tim 11-17-25 - regions don't have to be in allen atlas
        % and electrode groups need to be included so that the remaining
        ...
        session.brainRegions.(regionNames{iRegion}).electrodeGroups = sessionInfo.brainRegions.(regionNames{iRegion}).electrodeGroups;
        ...
        %         session.channelTags.(tagName).electrodeGroups = sessionInfo.brainRegions.(regionNames{iRegion}).electrodeGroups;
        %     end
        % end
    end
end
